...
def deploy_weekly_scraper(bucket_name: str, tavily_api_key: str = None):
    """Complete deployment of weekly diabetes scraper"""
    
    print("🚀 Deploying Weekly Diabetes Scraper")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    if not bucket_name:
        if len(sys.argv) < 2:
            print("Usage: python deploy_weekly_scraper.py [bucket_name] [tavily_api_key]")
            print("Or set S3_BUCKET_NAME environment variable")
            print("Example: python deploy_weekly_scraper.py my-diabetes-research-bucket")
            sys.exit(1)
        bucket_name = sys.argv[1]
        tavily_api_key = sys.argv[2] if len(sys.argv) > 2 else None
    else:
        tavily_api_key = sys.argv[1] if len(sys.argv) > 1 else None
        print(f"Using bucket from environment: {bucket_name}")
    
    success = deploy_weekly_scraper(bucket_name, tavily_api_key)
